...
contract masterSLEC is IERC777Recipient {
    ...
    */
    ...
    // Q: does this struct need more properties?
    struct buySpecs {
        uint id;
    }
    ...
    struct energyStream {
        address addr; // consumer or prosumer's public key
        uint64 timestamp; // timestamp for when the energy offer/ask was submitted
        uint startDeliveryTimestamp;
        uint endDeliveryTimestamp;
        uint price; // starting price before bids
        uint64 energy;  // energy requested or available
    }
    ...
    function register(participant memory member) public {
        require(participants[msg.sender].exists == 1, "already registered");
     ...
     * @param idOffer the ID of the highest offer?  Cannot be!
    ...
    function acceptHighestBid (uint idOffer) isParticipant(msg.sender) marketIsClosed() public returns(streamRef memory highestStream) {
        ...
        for (uint k = 0 ; k < bids[offerRef].length; k++) {
            ...
            energyStream memory stream = buyEnergy_map[ref.addr][ref.id];
            bytes memory bid_acquired = abi.encodePacked(msg.sender, idOffer, ref.id, ref.addr);
            if (price < stream.price && bids_acquired[bid_acquired] <= stream.price) {
                price = stream.price;
                highestStream = ref;
            }
        }
        ...
        // give back the deposit to bidder who didn't win
        for (uint k = 0 ; k < bids[offerRef].length; k++) {
            streamRef memory ref = bids[offerRef][k];
            if (ref.addr != highestStream.addr || ref.id != highestStream.id) {
                bytes memory bid_ref = abi.encodePacked(msg.sender, idOffer, ref.id, ref.addr);
                if (bids_acquired[bid_ref] != 0) {
                    token.send(ref.addr, bids_acquired[bid_ref], "");
    ...
    /* 
     * YANN - lets review this function & how it works with the tokensReceived function
    ...
    */
    ...
    function transactionCompleted (uint idOffer, address seller, uint idAsk) isParticipant(msg.sender) marketIsClosed() public {
        ...
        // release the funds to the producer
    ...
    function findAppropriateSellers (uint id) view public isParticipant(msg.sender) marketIsOpen() returns (streamRef[] memory matches) {
        // This should return all the sellEnergy offers that matches the buyEnergy for the current msg.sender
