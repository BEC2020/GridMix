...
contract KWH is ERC777 {
    constructor (address[] memory operators, address governor) ERC777("energy", "KWH", operators) public {
        _mint(governor, 1000000, bytes(""), bytes(""));
    }
}
...
contract masterSLEC is IERC777Recipient {
    ...
    */
    ...
    struct participant {
        ...
        location loc; // longlat - is location defined yet?
    ...
    mapping(bytes => uint) bids_acquired; // bytes is concatenation of seller - buyer - id offer - id ask
    ...
    function tokensReceived (
        ...
        bytes32 idAsk = abi.decode(userData[0:32], (bytes32));
        bytes32 idOffer = abi.decode(userData[32:64], (bytes32));
        bytes32 sellerAddress = abi.decode(userData[64:92], (bytes32));
       
        bytes memory ref = abi.encodePacked(sellerAddress, from, idOffer, idAsk);
    ...
    function acceptHighestBid (uint idOffer) isParticipant(msg.sender) marketIsClosed() public returns(streamRef memory highestStream) {
        ...
        for (uint k = 0 ; k < bids[offerRef].length; k++) {
            ...
            bytes memory bid_acquired = abi.encodePacked(msg.sender, ref.addr, idOffer, ref.id);
    ...
    /* 
     * This is called buy the buyer (either the grid or the local entity)
